...
const StepCharacters = () => {
  ...
  const createSupportingCharacters = async () => {
    if (hero && villain) {
      const promptSupportingNames = `Generate names for supporting characters for a ${context.genre} story with the plot of "${context.logline}". Respond with at least 3 names as a comma separated strings. Do not number the names.`
