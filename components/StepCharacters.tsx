...
const StepCharacters = () => {
	...
	const createSupportingCharacters = async () => {
		if (hero && villain) {
			const promptSupportingNames = `Generate names for supporting characters for a ${context.genre} story with the plot of "${context.logline}". Respond with the names as a comma separated strings.`
			...
			for (let i = 0; i < supportingArray.length; i++) {
				...
				const basePrompt = `Generate a short character description for a supporting character named ${supportingName} for a ${context.genre} movie based on the logline "${context.logline}". The main character is ${hero.name}. ${hero.description} The antagonist is ${villain.name}. ${villain.description}`
