...
const StepActs = () => {
	...
	useEffect(() => {
		const getTitle = async () => {
			const prompt = `Generate a movie title for ${context.genre
				} movie based on the logline "${context.logline}" ${context.acts ? "and three acts of " + context.acts.join(", ") : ""
				}`
	...
	return (
		<div className="relative z-10 max-w-3xl text-left mx-auto">
			...
			{context.acts.map((act, i) => (
				<div className="pb-8">
					<div className="text-3xl text-center font-bold pb-2">{`Act ${i + 1
						}`}</div>
			...
			<div className="text-center pt-4">
				<button
					...
					className={`bg-indigo-600 rounded-xl text-white font-medium text-xl sm:text-3xl py-4 pl-12 pr-16 mt-2 mb-4 ${context.genre && context.logline
							? ""
